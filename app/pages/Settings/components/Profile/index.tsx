...
class SettingsProfile extends React.Component<IProps & InjectedIntlProps, {}> {
  render() {
    ...
    <div>
      ...
      <p>{formatMessage(messages.changeAvatarBeforeLink)} <a target={"_blank"} rel={"noopener"} href={'https://gravatar.com'}>{formatMessage(messages.gravatarLinkText)} <Icon iconSize={"inherit"} iconName={"share"}/></a></p>
      ...
      <small>{formatMessage(messages.remarks)} <a target={"_blank"} rel={"noopener"} href={'https://github.com/keycloak/keycloak'}>{formatMessage(messages.keycloakSourceLinkText)} <Icon iconSize={"inherit"} iconName={"share"}/></a></small>
    </div>)
  }
}
